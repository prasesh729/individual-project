...
class ProfileForm(BasicForm):
    ...
    prefHospital = forms.ModelChoiceField(
        label="Preferred Hospital", required=False, queryset=Hospital.objects.all())
    ...
    speciality = forms.ModelChoiceField(
        label="Speciality", required=False, queryset=Speciality.objects.all())
    ...
    def clean(self):
        ...
        if startTime and endTime:
            if endTime <= startTime:
                self.mark_error(
                    'endTime', 'The appointment end time must come after the start time')
        return cleaned_data


class SpecialityForm(BasicForm):
    name = forms.CharField(label='Name of speciality', max_length=4000)
...
class SymptomForm(BasicForm):
    name = forms.CharField(label='Name of symptom', max_length=4000)
...
class EmployeeRegistrationForm(BasicForm):
    ...
    employee = forms.ChoiceField(
        required=False, choices=Account.EMPLOYEE_TYPES)
...
class MedicalInfoForm(BasicForm):
    account = forms.ModelChoiceField(
        label="Patient", queryset=Account.objects.filter(role=Account.ACCOUNT_PATIENT))
    setup_field(account)
    bloodType = forms.ChoiceField(
        label="Blood Type", choices=MedicalInfo.BLOOD, required=False)
...
class ExportForm(forms.Form):
    ...
    export = forms.ChoiceField(
        required=True, widget=forms.RadioSelect, choices=CHOICES)
