    ...
    if request.method == 'POST':
        if 'delete' in request.POST and 'pk' in request.POST:
            ...
            try:
                ...
                return
            speciality.delete()
            logger.log(Action.ACTION_ADMIN, 'Speciality cancelled',
    ...
    if request.method == 'POST':
        if 'delete' in request.POST and 'pk' in request.POST:
            ...
            try:
                ...
                return
            symptom.delete()
            logger.log(Action.ACTION_ADMIN, 'Symptom cancelled',
                       request.user.account)
...
def users_view(request):
    ...
    if request.method == 'POST':
        ...
        if account is not None:
            account.role = role
            account.save()
            logger.log(Action.ACTION_ADMIN, 'Admin modified ' +
                       account.profile.firstname + "'s role", request.user.account)
            template_data['alert_success'] = "Updated " + \
                account.profile.firstname + "'s role!"
...
def add_speciality(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.log(Action.ACTION_ADMIN, 'Admin added ' +
                       speciality.name, request.user.account)
            return HttpResponseRedirect('/admin/speciality')
    else:
        form = SpecialityForm()
...
def add_symptom(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.log(Action.ACTION_ADMIN, 'Admin added ' +
                       symptom.name, request.user.account)
            return HttpResponseRedirect('/admin/symptom')
    else:
        form = SymptomForm()
    template_data['form'] = form
...
def createemployee_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.log(Action.ACTION_ADMIN, 'Admin registered ' +
                       user.username, request.user.account)
...
def statistic_view(request):
    ...
    # Get the template data from the session
    template_data = views.parse_session(
        request, {'form_button': "Get Statistics"})
    # Proceed with the rest of the view
    default = {}
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            date_filter = Action.objects.all().filter(
                timePerformed__range=(statistics.startDate, statistics.endDate))
            ...
            template_data['total_logins'] = Action.objects.filter(
                description__icontains="Account login", timePerformed__range=(statistics.startDate, statistics.endDate)).count()
            template_data['total_logouts'] = Action.objects.filter(
                description__icontains="Account logout", timePerformed__range=(statistics.startDate, statistics.endDate)).count()
            template_data['total_appointments'] = Action.objects.filter(
                description__icontains="Appointment created", timePerformed__range=(statistics.startDate, statistics.endDate)).count()
            template_data['total_med_tests'] = Action.objects.filter(
                description__icontains="Medical Test created", timePerformed__range=(statistics.startDate, statistics.endDate)).count()
            template_data['total_registered'] = Action.objects.filter(
                description__icontains="registered", timePerformed__range=(statistics.startDate, statistics.endDate)).count()
        ...
        if form.is_valid():
            ...
            for line in file:
                ...
                if first_word == 'firstname':
                    ...
                    m = str(count[0])+' users are successfully uploaded, ' + \
                        str(count[1])+' duplicate accounts.'
                    if count[0] == 0:
                    ...
                    m = str(count[0])+' hospitals are successfully uploaded, ' + \
                        str(count[1])+' duplicate hospitals.'
                    if count[0] == 0:
                        template_data['alert_danger'] = m
                    else:
                        template_data['alert_success'] = m
...
def generate_user_csv():
    ...
    write = writer(response, delimiter=',',
